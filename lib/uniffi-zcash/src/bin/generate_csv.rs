...
fn main() {
    let mut file = OpenOptions::new()
        ...
        .write(true)
        .truncate(true)
        .open("uniffi-zcash/tests/test_data.csv")
        .unwrap();
    ...
    let mut line = sapling::extended_spending_key(&seed);
    writeln!(file, "{line}").unwrap();
    line = sapling::extended_spending_key_from_path(&seed);
    writeln!(file, "{line}").unwrap();
    line = sapling::extended_spending_key_derive_child(&seed);
    writeln!(file, "{line}").unwrap();
    line = sapling::extended_spending_key_default_address(&seed);
    writeln!(file, "{line}").unwrap();
    line = sapling::extended_spending_key_default_child_index(&seed);
    writeln!(file, "{line}").unwrap();
    line = sapling::extended_spending_key_derive_internal(&seed);
    writeln!(file, "{line}").unwrap();
    line = sapling::extended_spending_key_fvk(&seed);
    writeln!(file, "{line}").unwrap();

    orchard::spending_key(&key);
    orchard::spending_key_from_zip32_seed(&seed);
}
