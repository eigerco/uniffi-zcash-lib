...
use crate::{
    cli::CLIResult,
    helper::{clean_dir, cmd_success, in_file_template_replace, tmp_folder},
};
...
pub fn python(cfg: &PythonConfig) -> CLIResult<()> {
    ...
    copy(
        cfg.bindings_dir.join("libuniffi_zcash.so"),
        cfg.package_dir.join("zcash").join("libuniffi_zcash.so"),
    )?;
    copy(
...
pub fn ruby(cfg: &RubyConfig) -> CLIResult<()> {
    ...
    copy(
        cfg.bindings_dir.join("libuniffi_zcash.so"),
        cfg.package_dir.join("lib").join("libuniffi_zcash.so"),
    )?;
    copy(
