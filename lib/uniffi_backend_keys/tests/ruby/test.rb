...
class TestApk < Test::Unit::TestCase
  def test_unified_spending_key_from_seed
    ...
    assert_equal(unified_spending_key.to_bytes(
      BackendKeys::ZcashKeysEra::ORCHARD), expected)
  end
  ...
  def test_unified_full_viewing_key_encode
    ...
    assert_equal(unified_spending_key.to_unified_full_viewing_key()
        .encode(BackendKeys::ZcashConsensusParameters::MAIN_NETWORK), expected)
  end
  ...
  def test_account_priv_key_from_extended_priv_key
    ...
    extended_priv_key = BackendKeys::ZcashExtendedPrivKey.with_seed(seed)
    account_priv_key = BackendKeys::ZcashAccountPrivKey.from_extended_privkey(extended_priv_key)
